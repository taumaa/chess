...
bool ImGui::Button(const char* label, const ImVec2& size_arg, ImDrawFlags draw_flags)
{
    return ButtonEx(label, size_arg, ImGuiButtonFlags_None, draw_flags);
}
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos, ImGuiDockNode* dock_node, float height)
{
    ...
    if (dock_node)
        RenderArrowDockMenu(window->DrawList, bb.Min + ImVec2{0.f, 1.f} + ImVec2(0.354f, 0.275f) * g.FontSize, g.FontSize, text_col);
    else
        RenderArrow(window->DrawList, bb.Min + ImVec2{0.f, 1.f}, text_col, window->Collapsed ? ImGuiDir_Right : ImGuiDir_Down, 1.0f);
...
bool ImGui::SplitterBehavior(const ImRect& bb, ImGuiID id, ImGuiAxis axis, float* size1, float* size2, float min_size1, float min_size2, float hover_extend, float hover_visibility_delay, ImU32 bg_col)
{
    ...
    bb_render.Min += {1.f, 1.f}; // (JF) Make separator thinner
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, ImDrawFlags draw_flags)
{
    ...
    {
        // (JF) Allow nicely matching arrows along rounded text input
...
bool ImGui::DataTypeApplyFromText(const char* buf, ImGuiDataType data_type, void* p_data, const char* format, CustomParser custom_parser)
{
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags, CustomParser custom_parser)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (temp_input_is_active)
    {
        ...
        return TempInputScalar(frame_bb, id, label, data_type, p_data, format, is_clamp_input ? p_min : NULL, is_clamp_input ? p_max : NULL, custom_parser);
    }
    ...
    LockMousePos((flags & ImGuiSliderFlags_Vertical) ? ImGuiAxesMask_Y : ImGuiAxesMask_X);
...
bool ImGui::DragFloat(const char* label, float* v, float v_speed, float v_min, float v_max, const char* format, ImGuiSliderFlags flags, CustomParser custom_parser)
{
    return DragScalar(label, ImGuiDataType_Float, v, v_speed, &v_min, &v_max, format, flags, custom_parser);
}
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const bool grab_hovered = grab_bb.Contains(ImGui::GetMousePos());

    // Render grab
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : grab_hovered ? ImGuiCol_SliderGrabHovered : ImGuiCol_SliderGrab), style.GrabRounding);
...
bool ImGui::TempInputScalar(const ImRect& bb, ImGuiID id, const char* label, ImGuiDataType data_type, void* p_data, const char* format, const void* p_clamp_min, const void* p_clamp_max, CustomParser custom_parser)
{
    ...
    if (TempInputText(bb, id, label, data_buf, IM_ARRAYSIZE(data_buf), flags))
    {
        ...
        DataTypeApplyFromText(data_buf, data_type, p_data, format, custom_parser);
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags, ImDrawFlags draw_flags, CustomParser custom_parser)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (p_step == NULL)
    {
        if (InputText(label, buf, IM_ARRAYSIZE(buf), flags))
            value_changed = DataTypeApplyFromText(buf, data_type, p_data, format, custom_parser);
    }
    else
    {
        ...
        if (InputText("", buf, IM_ARRAYSIZE(buf), flags, nullptr, nullptr, draw_flags)) // PushId(label) + "" gives us the expected ID from outside point of view
            value_changed = DataTypeApplyFromText(buf, data_type, p_data, format, custom_parser);
...
bool ImGui::InputFloat(const char* label, float* v, float step, float step_fast, const char* format, ImGuiInputTextFlags flags, ImDrawFlags draw_flags)
{
    flags |= ImGuiInputTextFlags_CharsScientific;
    return InputScalar(label, ImGuiDataType_Float, (void*)v, (void*)(step > 0.0f ? &step : NULL), (void*)(step_fast > 0.0f ? &step_fast : NULL), format, flags, draw_flags);
}
...
bool ImGui::InputText(const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data, ImDrawFlags draw_flags)
{
    IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline()
    return InputTextEx(label, NULL, buf, (int)buf_size, ImVec2(0, 0), flags, callback, user_data, draw_flags);
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data, ImDrawFlags draw_flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    {
        RenderNavHighlight(frame_bb, id);
        RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBg), true, style.FrameRounding, draw_flags);
    }
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled, float arrow_offset_y)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    else
    {
        ...
        RenderArrow(window->DrawList, pos + ImVec2(offsets->OffsetMark + extra_w + g.FontSize * 0.30f, arrow_offset_y), GetColorU32(ImGuiCol_Text), ImGuiDir_Right);
    }
    if (!enabled)
        EndDisabled();
...
bool ImGui::BeginMenu(const char* label, bool enabled, float arrow_offset_y)
{
    return BeginMenuEx(label, NULL, enabled, arrow_offset_y);
}
